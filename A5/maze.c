...
struct floor* initMaze(int floorWidth, int floorHeight, int floorType){
    struct floor* toRet;
    int i;
    ...
    if(floorType==OUTSIDE){
        ...
        genOutdoors(toRet);
        return toRet;
    } else if(floorType==CAVE){
        // Set that this is a cave map
        toRet->floorType = CAVE;
        // Init heightmap
        for(i = 0; i < toRet->floorWidth; i++){
            toRet->heightMap[i] = (float*)malloc(toRet->floorHeight * sizeof(float));
            if(toRet->heightMap[i] == NULL){
        ...
        genCave(toRet);
        return toRet;
    } else if(floorType==DUNGEON){
        // Set that this is an outdoor map
        toRet->floorType = DUNGEON;
        // Init floorplan
        for(i = 0; i < toRet->floorWidth; i++){
            toRet->floorData[i] = (char*)malloc(toRet->floorHeight * sizeof(char));
            if(toRet->floorData[i] == NULL){
...
void genCave(struct floor* maze){
    if(DEBUG==0)
        printf("Generating cave floor...\n");

    struct position pen; // Point for writing to the maze
    int x, y;

    // First set entire cave to 'empty' space (for entity positioning)
    for(x = 0; x < maze->floorWidth; x++){
        for(y = 0; y < maze->floorHeight; y++){
            maze->floorEntities[x][y] = ' ';
    ...
    maze->drawDist = 33.0; // 1/3rd draw distance by default
